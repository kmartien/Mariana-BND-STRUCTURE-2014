...
msats <- gtypes(msat.merge, id.col = 1, strata.col = 4, locus.col = 10, description = "Ttru Run 7")

# Run STRUCTURE
sr <- structure.run(msats, k.range = 2:2, num.k.rep = 10, label = "Run 7", delete.files = TRUE, num.cores = 2, 
                    burnin = 100000, numreps = 500000, noadmix = FALSE, freqscorr = FALSE, 
                    pop.prior = "usepopinfo")

save.image("Ttru_Run7_sr.rdata")
